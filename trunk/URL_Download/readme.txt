    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_URL_Download.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * URL_Download.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * URL_Download.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * URL_Download (application created by MATLAB Compiler)
        * run_URL_Download.sh (shell script that can be run to 
    ...
    All Platforms:
        * URL_Download.dll (shared library, file extension varies by 
          platform)
        * URL_Download.h (library header file)
        * URL_Download.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * URL_Download.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * URL_Download.xml (documentation files)
        * URL_Download.pdb (program database file - if DEBUG option 
          selected)
        * URL_Download.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * URL_Download.ctf (Component Technology File archive, contains 
          compiled M-code)
        * URL_Download_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * URL_Download.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * URL_Download.ctf (Component Technology File archive, contains 
          compiled M-code)
        * URL_Download.xla (Optionally created and placed in
