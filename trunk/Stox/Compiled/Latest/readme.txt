    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_Stox.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * Stox.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * Stox.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * Stox (application created by MATLAB Compiler)
        * run_Stox.sh (shell script that can be run to 
    ...
    All Platforms:
        * Stox.dll (shared library, file extension varies by 
          platform)
        * Stox.h (library header file)
        * Stox.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * Stox.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * Stox.xml (documentation files)
        * Stox.pdb (program database file - if DEBUG option 
          selected)
        * Stox.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * Stox.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * Stox.ctf (Component Technology File archive, contains 
          compiled M-code)
        * Stox.xla (Optionally created and placed in
          C:\SourceSafe\Stocks & Shares\Programs\Stox\Compiled\Latest directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
