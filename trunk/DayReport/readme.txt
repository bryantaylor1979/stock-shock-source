    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_DayReport.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * DayReport.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * DayReport.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * DayReport (application created by MATLAB Compiler)
        * run_DayReport.sh (shell script that can be run to 
    ...
    All Platforms:
        * DayReport.dll (shared library, file extension varies by 
          platform)
        * DayReport.h (library header file)
        * DayReport.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * DayReport.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * DayReport.xml (documentation files)
        * DayReport.pdb (program database file - if DEBUG option 
          selected)
        * DayReport.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * DayReport.ctf (Component Technology File archive, contains 
          compiled M-code)
        * DayReport_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * DayReport.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * DayReport.ctf (Component Technology File archive, contains 
          compiled M-code)
        * DayReport.xla (Optionally created and placed in
