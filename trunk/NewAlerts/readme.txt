    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_NewsAlerts.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * NewsAlerts.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * NewsAlerts.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * NewsAlerts (application created by MATLAB Compiler)
        * run_NewsAlerts.sh (shell script that can be run to 
    ...
    All Platforms:
        * NewsAlerts.dll (shared library, file extension varies by 
          platform)
        * NewsAlerts.h (library header file)
        * NewsAlerts.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * NewsAlerts.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * NewsAlerts.xml (documentation files)
        * NewsAlerts.pdb (program database file - if DEBUG option 
          selected)
        * NewsAlerts.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * NewsAlerts.ctf (Component Technology File archive, contains 
          compiled M-code)
        * NewsAlerts_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * NewsAlerts.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * NewsAlerts.ctf (Component Technology File archive, contains 
          compiled M-code)
        * NewsAlerts.xla (Optionally created and placed in
