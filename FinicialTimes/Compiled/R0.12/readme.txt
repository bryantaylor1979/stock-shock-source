    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_FT_BrokersView.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * FT_BrokersView.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * FT_BrokersView.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * FT_BrokersView (application created by MATLAB Compiler)
        * run_FT_BrokersView.sh (shell script that can be run to 
    ...
    All Platforms:
        * FT_BrokersView.dll (shared library, file extension varies by 
          platform)
        * FT_BrokersView.h (library header file)
        * FT_BrokersView.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * FT_BrokersView.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * FT_BrokersView.xml (documentation files)
        * FT_BrokersView.pdb (program database file - if DEBUG option 
          selected)
        * FT_BrokersView.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * FT_BrokersView.ctf (Component Technology File archive, contains 
          compiled M-code)
        * FT_BrokersView_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * FT_BrokersView.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * FT_BrokersView.ctf (Component Technology File archive, contains 
          compiled M-code)
        * FT_BrokersView.xla (Optionally created and placed in
