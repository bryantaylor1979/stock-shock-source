    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_FinicialTimes.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * FinicialTimes.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * FinicialTimes.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * FinicialTimes (application created by MATLAB Compiler)
        * run_FinicialTimes.sh (shell script that can be run to 
    ...
    All Platforms:
        * FinicialTimes.dll (shared library, file extension varies by 
          platform)
        * FinicialTimes.h (library header file)
        * FinicialTimes.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * FinicialTimes.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * FinicialTimes.xml (documentation files)
        * FinicialTimes.pdb (program database file - if DEBUG option 
          selected)
        * FinicialTimes.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * FinicialTimes.ctf (Component Technology File archive, contains 
          compiled M-code)
        * FinicialTimes_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * FinicialTimes.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * FinicialTimes.ctf (Component Technology File archive, contains 
          compiled M-code)
        * FinicialTimes.xla (Optionally created and placed in
          C:\SourceSafe\Stocks & Shares\Programs\FinicialTimes\Compiled\R0.15 directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
