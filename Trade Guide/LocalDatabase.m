...
%Date Modified: 2nd Feb 2009
    properties
       ...
       LastPrice
       LastPriceDate
       ExternalCommObj = 'yahoo-fetch'; %yahoo-fetch or sqq
    ...
    methods (Hidden = false)
        function [obj,DataStore] = LoadData(obj)
        ...
        function [obj,StartDate,EndDate] = DateRange(obj)
        ...
        function [obj,SymbolsList] = GetDownloadedSymbolList(obj)
            logdir = pwd;
            SaveDataPath = [obj.Location,obj.DatabaseName];
            path = [SaveDataPath,'\Download_Mat\Data\'];
            cd(path);
        ...
        function [Info] = Sync(obj)
              ...
              [obj,LocalStatus] = obj.DetermineDownloadState();
              ...
              if LocalStatus == 1 %%UPDATE
                  ...
                  [obj,Status] = obj.RequireSync();
                  startdate = obj.StartDate;
                  enddate = obj.EndDate;

                  if strcmpi(Status,'OutOfDate')
                    DataStore = obj.DataStore;
                    [obj,NewDataStore,timedout,Info] = obj.DownloadData([obj.EndDate+1,today]);
                  ...
                  [obj,DataStore,timedout,Info] = obj.DownloadData('all');
        ...
        function [Data] = GetRange(obj,Name,Period)
            switch lower(Period)
                case '1w'
                    [Data] = GetData(obj,Name,[today,today-7]);
                case '2w'
                    [Data] = GetData(obj,Name,[today,today-14]);
                case '1m'
                    [Data] = GetData(obj,Name,[today,today-30]);
                case '3m'
                    [Data] = GetData(obj,Name,[today,today-120]);
                case '6m'
                    [Data] = GetData(obj,Name,[today,today-6*30]);
                case '1y'
                    [Data] = GetData(obj,Name,[today,today-365]);
                otherwise
            end
        end
        function [obj] = GetLastPrice(obj);
            try
            [Data] = fetch(obj.conn,obj.Symbol,{'Last';'Date'});
            catch
            pause(5)
            obj = obj.Connect2yahoo;
            [Data] = fetch(obj.conn,obj.Symbol,{'Last';'Date'});
            end
    ...
    methods (Hidden = false) % Calculations
        ...
        function [obj,Status] = CalcChange(obj)
        ...
        function [obj,Status] = CalcPriceMean(obj) %Verfied
            if isempty(obj.DataStore);
                obj = obj.LoadData();
            end
        ...
        function [obj,Status] = CalcMovAvg(obj,NumberOfDays)
            if isempty(obj.DataStore);
                obj = obj.LoadData();
            end
        ...
        function [obj,Status] = CalcFiveDayHigh(obj)
            if isempty(obj.DataStore);
                obj = obj.LoadData();
            end
        ...
        function [obj,Status] = CalcDayDiff(obj)
            if isempty(obj.DataStore);
                obj = obj.LoadData();
            end
        ...
        function [obj,Status] = PercentageChange(obj)
            if isempty(obj.DataStore);
                obj = obj.LoadData();
            end
        ...
        function [obj,Status] = VolumeDiff(obj)
            if isempty(obj.DataStore);
                obj = obj.LoadData();
            end
        ...
        function [obj,Status] = PriceMean_DeNoise(obj)
            if isempty(obj.DataStore);
                obj = obj.LoadData();
            end
        ...
        function [obj,Status] = TradeSignal(obj)
            if isempty(obj.DataStore);
                obj = obj.LoadData();
            end
    ...
    methods (Hidden = true) % Support functions
        ...
        function [obj,DataStore,timedout,Info] = DownloadData(obj,DateRange)
        % DataRange = 'all' or [today-7,today];
            if ischar(DateRange)
...
%                     DataStore(1:i,j+1) = NaN;
        ...
        function [obj,Status] = DetermineDownloadState(obj)
        ...
        function [obj,Status] = RequireSync(obj)
            ...
            if strcmpi(obj.DebugReport,'on')
                 ...
                 disp(['Today Date: ',datestr(today),' LastWorkingDay: ',datestr(LastWorkingDay),' Status: ',Status])
                 disp(' ')
            end
        end
        ...
        function [obj] = LocalDatabase(obj)
            obj = obj.Connect2yahoo;
        end
        function [obj] = Connect2yahoo(obj)
            Time = 1;
            Timeout = 20;
            while Time < Timeout
                try
