    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_CheckConnection.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * CheckConnection.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * CheckConnection.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * CheckConnection (application created by MATLAB Compiler)
        * run_CheckConnection.sh (shell script that can be run to 
    ...
    All Platforms:
        * CheckConnection.dll (shared library, file extension varies by 
          platform)
        * CheckConnection.h (library header file)
        * CheckConnection.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * CheckConnection.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * CheckConnection.xml (documentation files)
        * CheckConnection.pdb (program database file - if DEBUG option 
          selected)
        * CheckConnection.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * CheckConnection.ctf (Component Technology File archive, contains 
          compiled M-code)
        * CheckConnection_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * CheckConnection.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * CheckConnection.ctf (Component Technology File archive, contains 
          compiled M-code)
        * CheckConnection.xla (Optionally created and placed in
