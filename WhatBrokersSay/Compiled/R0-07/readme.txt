    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_WhatBrokersSay.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * WhatBrokersSay.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * WhatBrokersSay.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * WhatBrokersSay (application created by MATLAB Compiler)
        * run_WhatBrokersSay.sh (shell script that can be run to 
    ...
    All Platforms:
        * WhatBrokersSay.dll (shared library, file extension varies by 
          platform)
        * WhatBrokersSay.h (library header file)
        * WhatBrokersSay.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * WhatBrokersSay.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * WhatBrokersSay.xml (documentation files)
        * WhatBrokersSay.pdb (program database file - if DEBUG option 
          selected)
        * WhatBrokersSay.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * WhatBrokersSay.ctf (Component Technology File archive, contains 
          compiled M-code)
        * WhatBrokersSay_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * WhatBrokersSay.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * WhatBrokersSay.ctf (Component Technology File archive, contains 
          compiled M-code)
        * WhatBrokersSay.xla (Optionally created and placed in
