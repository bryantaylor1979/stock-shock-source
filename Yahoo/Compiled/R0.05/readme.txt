    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_Yahoo.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * Yahoo.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * Yahoo.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * Yahoo (application created by MATLAB Compiler)
        * run_Yahoo.sh (shell script that can be run to 
    ...
    All Platforms:
        * Yahoo.dll (shared library, file extension varies by 
          platform)
        * Yahoo.h (library header file)
        * Yahoo.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * Yahoo.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * Yahoo.xml (documentation files)
        * Yahoo.pdb (program database file - if DEBUG option 
          selected)
        * Yahoo.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * Yahoo.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * Yahoo.ctf (Component Technology File archive, contains 
          compiled M-code)
        * Yahoo.xla (Optionally created and placed in
          C:\SourceSafe\Stocks & Shares\Programs\Yahoo\Compiled\R0.05 directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
