    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_Download_mat.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * Download_mat.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * Download_mat.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * Download_mat (application created by MATLAB Compiler)
        * run_Download_mat.sh (shell script that can be run to 
    ...
    All Platforms:
        * Download_mat.dll (shared library, file extension varies by 
          platform)
        * Download_mat.h (library header file)
        * Download_mat.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * Download_mat.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * Download_mat.xml (documentation files)
        * Download_mat.pdb (program database file - if DEBUG option 
          selected)
        * Download_mat.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * Download_mat.ctf (Component Technology File archive, contains 
          compiled M-code)
        * Download_mat_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * Download_mat.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * Download_mat.ctf (Component Technology File archive, contains 
          compiled M-code)
        * Download_mat.xla (Optionally created and placed in
