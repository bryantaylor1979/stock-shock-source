    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_WebPageDecoder.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * WebPageDecoder.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * WebPageDecoder.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * WebPageDecoder (application created by MATLAB Compiler)
        * run_WebPageDecoder.sh (shell script that can be run to 
    ...
    All Platforms:
        * WebPageDecoder.dll (shared library, file extension varies by 
          platform)
        * WebPageDecoder.h (library header file)
        * WebPageDecoder.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * WebPageDecoder.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * WebPageDecoder.xml (documentation files)
        * WebPageDecoder.pdb (program database file - if DEBUG option 
          selected)
        * WebPageDecoder.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * WebPageDecoder.ctf (Component Technology File archive, contains 
          compiled M-code)
        * WebPageDecoder_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * WebPageDecoder.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * WebPageDecoder.ctf (Component Technology File archive, contains 
          compiled M-code)
        * WebPageDecoder.xla (Optionally created and placed in
